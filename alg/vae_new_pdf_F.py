...
def probxy(x, enc, dec, ll, dimY, lowerbound, K = 1, beta=1.0, categorical=False):
    ...
    for i in range(dimY):
        ...
        logpxy.append(bound_sum/mctimes)
    pxy = tf.math.exp(logpxy)
    return logpxy, pxy
...
def construct_optimizer(X_ph, Y_ph, enc, dec, ll, K, vae_type='A', categorical=False):
    ...
    if vae_type == 'F':
        from alg.lowerbound_functions_F import lowerbound_F as lowerbound_func
    # if vae_type == 'G':
    ...
    logpxy, pxy = probxy(X_, enc, dec, ll_, dimY, lowerbound_func, K=10, beta=beta_ph, categorical=categorical) # TODO: weired
    ...
    def eval_categorical(sess, X, Y, data_name = 'train', beta=1.0):
        ...
        for j in range(0, n_batch):
            ...
            res_logpxy, res_pxy, res1, res2 = sess.run((logpxy, pxy, logprob, bound), feed_dict={X_ph:X[indl:indr],
                                                                 Y_ph: Y[indl:indr],
        ...
        print("%s data approx log likelihood logpxy=%.2f, bound=%.2f, time=%.2f, beta=%.2f" \
